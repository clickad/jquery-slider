    ...
    var app = {
      ...
      runSlides: function runSlides() {
        ...
        $('.slider__wrapper > div:first').fadeOut(this.$fadeTime, function () {
          $(_this).find(_this.$sliderTitle).animate({ marginTop: '-999px' });
          $(_this).find(_this.$sliderText).delay(_this.$fadeTime).animate({ marginLeft: '3999px' });
          $(_this).find(self.$sliderLink).delay(self.$fadeTime * 2).animate({ opacity: '0' });
